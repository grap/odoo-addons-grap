...
class change_account_move_line_wizard(TransientModel):
    ...
    def default_get(self, cr, uid, fields, context):
        res = super(change_account_move_line_wizard, self).default_get(cr, uid, fields, context=context)
        account = self.pool.get('account.account').browse(cr, uid, context['active_id'], context=context)
        ...
        res.update({
            ...
            'source_closed_period_move_number': account.closed_period_move_number,
    ...
    # --- Columns
    _columns = {
        'source_account_id' : fields.many2one('account.account', 'Source account', domain=[('type','=','view')], readonly=True),
        'source_company_id' : fields.many2one('res.company', 'Company of the source account', readonly=True),
        'source_type' : fields.char('Type of the source account', readonly=True),
        'source_move_number' : fields.integer('Number of moves in the source account', readonly=True),
        'source_invoice_number' : fields.integer('Number of invoices in the source account', readonly=True),
        'source_voucher_line_number' : fields.integer('Number of voucher line in the source account', readonly=True),
        'source_reconciled_move_number' : fields.integer('Number of reconciled moves in the source account', readonly=True),
        'source_closed_period_move_number' : fields.integer('Number of moves in closed periods', readonly=True),
        'destination_account_id' : fields.many2one('account.account', 'Destination account', required=True, domain=[('type','!=','view')]),
        'destination_company_id' : fields.many2one('res.company', 'Company of the destination account', readonly=True),
        'destination_type' : fields.char('Type of the destination account', readonly=True),
        'destination_move_number' : fields.integer('Number of moves in the destination account', readonly=True),
        'change_view_type' : fields.boolean('Change source account in view type'),
        'line_ids': fields.one2many('change.account.move.line.wizard.line', 'wizard_id', 'Partners list'),
    }
    ...
    def onchange_destination_account_id(self, cr, uid, ids, 
        ...
        if destination_account_id:
            account = self.pool.get('account.account').browse(cr, uid, destination_account_id, context=context)
            return {'value': {
    ...
    def button_change_move_line(self, cr, uid, ids, context=None):
        ...
        for data in self.browse(cr, uid, ids, context=context):
            # check constraint
            if not (data.source_account_id.company_id.id != data.destination_account_id.company_id.id):
                assert 'Incorrect request', _('Source and destination account must be belong to the same company.')
            ...
            if aml_ids: 
                ...
                cr.execute("SELECT distinct(move_id) FROM account_move_line WHERE id in (%s)"
                    % (str(aml_ids).strip('[]'),))
                move_ids = [r[0] for r in cr.fetchall()]

                # backup reconcile information
                for aml in aml_obj.browse(cr, uid, aml_reconcile_ids, context=context):
                ...
                aml_obj.write(cr, uid, aml_ids, {
                    'account_id': data.destination_account_id.id,}, context=context)
                ...
                for line_id in data.line_ids:
                    ...
                    aml_obj.write(cr, uid, aml_partner_ids, {
                        'partner_id': line_id.partner_id.id,}, context=context)
                ...
                for aml_reconcile_id in  aml_reconcile_ids:
                    aml_obj.write(cr, uid, aml_reconcile_id, {
                        'reconcile_id': reconcile_infos[aml_reconcile_id],}, context=context)
                
                # Close periods
                # NOTA : A function doesn't exist to do that. (except in a transient model)
                if close_period_infos:
                    mode = 'done'
                    cr.execute('update account_journal_period set state=%s where period_id in %s', (mode, tuple(close_period_infos),))
                    cr.execute('update account_period set state=%s where id in %s', (mode, tuple(close_period_infos),))
...
class change_account_move_line_wizard_line(TransientModel):
    ...
    _columns = {
        'wizard_id': fields.many2one('change.account.move.line.wizard', 'Wizard Reference', select=True),
        'company_id' : fields.many2one('res.company', 'Company',), #readonly=True
        'move_number' : fields.integer('Number of moves linked to the company', readonly=True),
