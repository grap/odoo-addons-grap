...
class account_account(Model):
    ...
    def _search_child_number(self, cr, uid, obj, name, arg, context=None):
        if type(arg[0][2]) == bool:
            raise except_orm(
                _("expression is not what we expect !"), str(arg))
        sql_req = """
        SELECT * FROM (
            SELECT aa.id,
                CASE WHEN calc.quantity is null
                    THEN 0 ELSE calc.quantity
                END as quantity
    ...
    def _search_account_ir_property_number(
        ...
        sql_req = """
            SELECT
                id,
                CASE WHEN tmp.quantity is null
                    THEN 0 ELSE tmp.quantity END as quantity
            FROM account_account
            LEFT OUTER JOIN (
                SELECT account_id, count(*) as quantity
                FROM (
                    SELECT
                        CAST(substring(value_reference from 17) AS INTEGER)
                        AS account_id
                    FROM ir_property
        ...
        for account in self.browse(cr, uid, ids, context=context):
            for ir_property in account.account_ir_property:
                self.pool.get('ir.property').unlink(
                    cr, uid, ir_property.id, context=context)
        return True
            ...
            if line_obj.search(cr, uid, [('account_id', 'in', account_ids)]):
                ...
                if old_type == 'closed' and new_type != 'closed':
                    raise osv.except_osv(_('Warning!'), _(
                        """You cannot change the type of account from"""
                        """ 'Closed' to any other type as it contains"""
                        """ journal items!"""))
                # Forbid to change an account type for restricted_groups
                # as it contains journal items
    ...
    def _check_allow_code_change(self, cr, uid, ids, context=None):
        ...
        for account in self.browse(cr, uid, ids, context=context):
            # BEGIN OF CHANGE
            # account_ids = self.search(
            #    cr, uid, [('id', 'child_of', [account.id])], context=context)
            account_ids = [account.id]
            # END OF CHANGE
            if line_obj.search(cr, uid, [
                    ('account_id', 'in', account_ids)], context=context):
                raise osv.except_osv(_('Warning !'), _(
                    """You cannot change the code of account which contains"""
                    """ journal items!"""))
        return True
