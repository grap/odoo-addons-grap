...
class fix_account_move_period_wizard(TransientModel):
    ...
    def default_get(self, cr, uid, fields, context):
        ...
        res = super(fix_account_move_period_wizard, self).default_get(cr, uid, fields, context=context)
        ...
        for am in am_obj.browse(cr, uid, am_ids, context=context):
            ap_id = am_obj._get_period_from_date_company_id(cr, uid, am.date, am.company_id.id, context=context)
    ...
    ### Columns
    _columns = {
        'line_ids': fields.one2many('fix.account.move.period.line.wizard', 'wizard_id', 'Account Moves list'),
    }
    ...
    def apply_period_change(self, cr, uid, ids, context=None):
        ...
        for fampw in self.browse(cr, uid, ids, context=context):
            ...
            for line in fampw.line_ids:
                am_obj.write(cr, uid, [line.account_move_id.id], {'period_id': line.new_period_id.id,}, context=context)
...
#    _rec_name = 'account_move_id'
    ...
    ### Columns section
    _columns = {
        'wizard_id': fields.many2one('fix.account.move.period.wizard', 'Wizard Reference', select=True),
        'account_move_id': fields.many2one('account.move', 'Account Move to fix', readonly=True),
        'company_id': fields.many2one('res.company', 'Company', readonly=True),
        'date': fields.date('Move Date', readonly=True),
        'old_period_id': fields.many2one('account.period', 'Old Period', readonly=True),
        'old_fiscalyear_id': fields.many2one('account.fiscalyear', 'Old Fiscal Year', readonly=True),
        'new_period_id': fields.many2one('account.period', 'New Period', required=True),
        'new_fiscalyear_id': fields.many2one('account.fiscalyear', 'New Fiscal Year', required=True),
    }
