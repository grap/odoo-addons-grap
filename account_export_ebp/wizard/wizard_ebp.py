...
#Columns Section
    _columns = {
        ...
        'company_suffix': fields.boolean("Append company's code to accounts",
            help = "When this is checked, the company's code will be appended to the receivable and payable accounts' numbers in the exported files on every move line.",),
        'partner_accounts': fields.boolean("Append partners' code to accounts",
            help = "When this is checked, the partner's special code will be appended to the receivable and payable accounts' numbers in the exported files on every move line where a partner has been specified.",),
        'tax_code_suffix': fields.boolean("Export according to Tax Codes",
            help = "Append Tax Code's suffix to account if the option is checked in the account"),
        ...
        'empty_suffixes_partner': fields.boolean('Empty Suffixes Partners', readonly=True),
        'empty_suffixes_tax': fields.boolean('Empty Suffixes Taxes', readonly=True),
    }
...
#Defaults Section
    ...
    def _get_empty_suffixes_tax(self, cr, uid, context=None):
        ...
        atc_ids = atc_obj.search(cr, uid, [('ref_nb', '=', False)], context=context)
        aml_ids = aml_obj.search(cr, uid, 
    ...
    def _download(self, cr, uid, ids, context=None):
        ...
        self.write(cr, uid, ids, {'state': 'export_ebp_download',
                                  ...
                                  'name_balance':this.name_balance}, context=context)
...
#        Move File header
        ...
        for move in moves:
            ...
            for line in move.line_id:
                ...
                if tax_code_suffix and line.account_id.export_tax_code and line.tax_code_id.ref_nb:
                ...
                if account_nb not in accounts_data.keys():
                    if (data['form']['partner_accounts']
                        ...
                        accounts_data[account_nb] = {
                            ...
                            'address': normalize((partner.street or '') + (partner.street2 and (' ' + partner.street2) or '')),
                            'zip': partner.zip or '',
                    ...
                    elif tax_code_suffix and line.account_id.export_tax_code and line.tax_code_id.ref_nb:
                        accounts_data[account_nb] = {
                            'name': normalize(line.account_id.name) + '(' + normalize(line.tax_code_id.name) + ')',
                            'partner_name': '',
                            'address': '',
                            'zip': '',
                            'city': '',
        ...
        for account_nb, account in accounts_data.iteritems():
            ...
            line2 = ','.join([
                ...
                str(debit), # Debit
                str(credit), # Credit
                str(debit_balance), # debit Balance
                str(credit_balance), # credit Balance
            ])
            balance_file.write(unidecode(line2))
            balance_file.write('\r\n')
