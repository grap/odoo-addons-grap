...
#Columns section
    def _export_name(self, cr, uid, ids, field_name, arg, context=None):
        export = self.pool.get('ebp.export')
        result = {}
        for move in self.browse(cr, uid, ids, context=context):
                result[move.id] = move.exported_ebp_id and 'export_' + str(move.exported_ebp_id.id) or ''
        return result

    _columns = {
        'exported_ebp_id': fields.many2one('ebp.export', 'Transfer id', 
            help="""Indicates whether the move has already been exported to EBP or not. It is changed automatically."""),
        'exported_ebp': fields.function(_export_name, type='char', string='export id', store=False),
    }
...
#Override section
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'exported_ebp_id' not in vals:
            exported_move_ids = self.search(cr, uid, [('exported_ebp_id', '!=', False), ('id', 'in', ids)])
