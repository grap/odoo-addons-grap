...
#Columns section
    def _nb_moves(self, cr, uid, ids, name, arg, context=None):
        ...
        for id in ids:
            aml_ids = aml_obj.search(cr, uid, [('partner_id', '=', id),('date', '>=', '01/12/2012')], context=context)
            res[id] = len(aml_ids)
        return res
    ...
    def _search_nb_moves(self, cr, uid, obj, name, args, context=None):
        if not args:
            return []
        query, query_args = self._get_search_moves_query(cr, uid, args, overdue_only=False, context=context)
    ...
    _columns = {
        ...
        'ref_nb': fields.char("Partner's Account suffix in EBP", size=4,
            help="When exporting Entries to EBP, this suffix will be appended\
                 to the Account Number to make it a Partner Account.",),
        'nb_moves': fields.function(_nb_moves, string = 'Number of moves',
...
#Constraints section
    _sql_constraints = [
        ('partner_suffix_uniq', 'unique (ref_nb, company_id)', 'The partner suffix must be unique per company!')
    ]
...
##Private section
    def _get_search_moves_query(self, cr, uid, args, overdue_only=False, context=None):
        having_where_clause = ' AND '.join(map(lambda x: '(COUNT(*) %s %%s)' % (x[1]), args))
        having_values = [x[2] for x in args]
        return "SELECT partner_id, count(*) \
