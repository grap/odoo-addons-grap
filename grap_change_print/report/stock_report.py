...
class ParserProductSummary(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(ParserProductSummary, self).__init__(
...
class ParserDeliveryOrder(report_sxw.rml_parse):

    def coincoin(self):
        return "COINCOIN"

    def invoice_address(self, picking):
        if picking.sale_id:
    ...
    def __init__(self, cr, uid, name, context):
        super(ParserDeliveryOrder, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            'invoice_address': self.invoice_address,
            'generated_password': 'COUCOU',
        })


report_sxw.report_sxw('report.webkit.aggregated_picking',
                      'stock.picking',
                      'addons/grap_change_print/report/picking.html.mako',
                      parser=PrintPick)

report_sxw.report_sxw('report.webkit.product_summary',
                      'stock.picking',
                      'addons/grap_change_print/report/stock_picking_out_product_summary.mako',
                      parser=ParserProductSummary)

report_sxw.report_sxw('report.webkit.delivery_order',
                      'stock.picking',
                      'addons/grap_change_print/report/stock_picking_out_delivery_order.mako',
                      parser=ParserDeliveryOrder)
